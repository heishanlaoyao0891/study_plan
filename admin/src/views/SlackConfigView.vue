...
const NumberField = defineComponent({
  props: { label: { type: String, required: true }, modelValue: { type: Number, required: true } },
  emits: ['update:modelValue'],
  setup(props, { emit }) {
    return () => h('label', { class: 'field' }, [
      h('span', props.label),
      h('input', { type: 'number', min: '0', value: props.modelValue, onInput: (event: Event) => emit('update:modelValue', Number((event.target as HTMLInputElement).value)) }),
    ])
  },
})
...
const global = reactive({ checkin_minutes: 10, streak_bonus: 0, quality_bonus: 0 })
const userConfig = reactive({ checkin_minutes: 10, streak_bonus: 0, quality_bonus: 0 })
